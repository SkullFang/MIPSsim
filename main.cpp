...
void disassembly(vector<string>& va);
...
vector<string> unit(2,"no"); //0-wait , 1-execute IF的两个单元
vector<string> pre_issue_queue(4,"no"); //pre_issue_queue队列
int sum_pre_issue_queue = 0; //pre_issue_queue队列的使用个数

vector<string> pre_alu_queue(2,"no"); //pre_lu_queue 队列
int sum_pre_alu_queue = 0; //alu队列的使用个数

string pre_mem_queue = "no"; //pre_mem
string post_mem_queue = "no";//post_mem
string post_alu_queue = "no";//post_alu

vector<string> sum_temp(5,"no");
vector<string> sum_temp2(9,"no");
...
void disassembly(vector<string>& va)
{
    ...
    for(i=0;i<va.size();++i)//一行一行的读
    {
        ...
        else if(start_0 == "000")// 一些跳转指令
        {
            ...
            else if(start_3 =="010")  //BEQ  //当两个寄存器中值相等跳转
            {
                va[i] += "BEQ";
                va[i] += ' ';
...
//                break;
            ...
            else if(start_3 == "111") //读，从存储器读到寄存器
            {
                va[i] += "LW";
                va[i] += ' ';
...
void simulation(vector<string> &va,int start)
{
    ...
    cout<<" index"<<im_index<<endl;

    //simulation开始
//    int im_index=156;
    int cycle_count = 0; // cycle
    bool can_fetch = true; //能不能fetch指令
    bool can_issue = true; // 能不能issue指令
    ...
    int temp_reg2;
    int temp_reg3;
//    string temp_pre_issue[4];
//    string temp_pre_alu[2];
//    string temp_pre_mem;
//    string temp_post_mem;
//    string temp_post_alu;
...
//    int j;
    ...
    //temp_temp
    int temp_sum_pre_issue_queue = 0; //issue_queue用的个数
    ...
    while(pc > 0)
    {
        ...
        for(int j = 0; j < sum_temp2.size(); j++) //
        {// 设置rs寄存器的状态为写，避免WAW，WAR
            if(sum_temp2[j] != "no")
            {
         ...
         *
         */
        if(post_alu_queue != "no")//得看看post_alu_que里面空不空
        {//如果有东西 就要去执行了
        ...
        */
         ...
         */
        have_store = false;
        if(pre_alu_queue[0] != "no") //pre_alu_queue是满的
        {
            can_issue = false;//
        }
        if(sum_pre_issue_queue == 0)//
        {
            can_issue = false;
        }
        if(can_issue)
        {
            ...
            for(int j = 0; j < sum_pre_issue_queue; j++)
            {
                ...
                if(temp_str4 == "ADDI" || temp_str4 == "ANDI" || temp_str4 == "XORI" || temp_str4 == "ORI ")
                {
                    ...
                    if( reg_state[temp_reg1] == 0 || reg_state[temp_reg2] == 0)//这个东西在上一轮周期之前更新过遍历的是ALU，mem里面的指令
                    {// rt rs 寄存器状态在被写
                        can_issue_pre[j] = false; //不能issue
                        if(reg_state[temp_reg1] != 0)
                        {
                            reg_state[temp_reg1] = 0; //rt改成自己要写  这是为了避免issue队列里面产生WAW，WAR
                        }
                ...
                if(temp_str4 == "SW R")//
                {
                    ...
                    if( reg_state[temp_reg1] == 0 || reg_state[temp_reg2] == 0)
                    {
                        ...
                        if(reg_state[temp_reg1] != 0)
                        {
                            reg_state[temp_reg1] = 2;  //read?  0? 2?
                        }
                        if(reg_state[temp_reg2] != 0)
                        {
                            reg_state[temp_reg2] = 2;  //read
            ...
            */
            ...
            for(int j = 0; j < sum_pre_issue_queue && can_issue_count > 0; j++)
            {
                if(can_issue_pre[j])
                {
                    ...
                    pre_issue_queue[sum_pre_issue_queue-1] = "no"; // can use i?
                    sum_pre_issue_queue--;
                    j--;
                }
            }
        ...
        */
        can_fetch = true;
        if(unit[0] != "no" || unit[1] != "no") // have waiting instruction of branch
        {
            can_fetch = false;
        } //分支等待。
...
//
//                    //get rs
//                    string start_3 = sum_temp[j].substr(3,5);
//                    int rs_value = 0;
//                    for(int i = 0;i< start_3.size();++i)
//                    {
//                        if(start_3[i] == '1')
...
//                    temp_reg2 = rs_value;
//
//                    //temp_reg1 = pre_issue_queue[j][5] - '0';
//                    //temp_reg2 = pre_issue_queue[j][9] - '0';
//
//
//                    reg_state[temp_reg1] = 0;
//
//                }//END OF ORI
                ...
                if(temp_str4 == "SW R")
                {
                    ...
                    string start_6 = sum_temp[j].substr(6,5);
                    int rs_value = 0;
                    for(int i = 0;i< start_6.size();++i)
                    {
                        if(start_6[i] == '1')
